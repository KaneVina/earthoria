...
export default function ChildActionsModal({ child, onClose, onLock, onUnlock, onDelete }) {
  useEffect(() => {
    const onKey = (e) => {
  ...
  return (
    ...
    >
      <div className="pf-confirm pkd-modal pkd-modal-wide pkd-actions-modal" role="dialog" aria-modal="true">
        ...
        <div className="pkd-actions-modal-head">
          ...
          <div>
            ...
            <span className={`pkd-actions-modal-status ${child.isLocked ? "is-locked" : "is-active"}`}>
              {child.isLocked ? (
                <>
        ...
        <div className="pkd-actions-modal-footer">
          {child.isLocked ? (
            <button className="pkd-lock-btn is-unlock pf-btn-tactile" onClick={onUnlock} type="button">
              <Unlock size={15} /> Mở khoá cho {child.name}
            </button>
          ) : (
            <button className="pkd-lock-btn is-lock pf-btn-tactile" onClick={onLock} type="button">
              <Lock size={15} /> Khoá ngay cho {child.name}
            </button>
          )}
          <button className="pkd-actions-modal-delete" onClick={onDelete} type="button">
