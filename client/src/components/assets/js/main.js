...
function animateStat(item) {
  ...
  function update(now) {
    ...
    if (progress < 1) requestAnimationFrame(update);
      ...
      countEl.textContent =
        target >= 100 ? target.toLocaleString("vi-VN") : target;
  }
...
const statsObserver = new IntersectionObserver(
  ...
  { threshold: 0.3 },
); // cần thấy 30% section mới kích hoạt
...
function handleFormSubmit(e) {
  ...
  btn.innerHTML =
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="20 6 9 17 4 12"/></svg> Đã gửi thành công!';
