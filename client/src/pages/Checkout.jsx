...
function VoucherModal({
  ...
  return (
    ...
    >
      <style>{`
        .eo-voucher-scroll::-webkit-scrollbar { width: 6px; }
        .eo-voucher-scroll::-webkit-scrollbar-track { background: transparent; }
          ...
          {loading ? (
            ...
            <div style={{ padding: "56px 0", textAlign: "center" }}>
              <Ticket
                size={30}
                strokeWidth={1}
                style={{ color: "var(--border-gold)", marginBottom: 12 }}
              />
              <div
                style={{
                  fontSize: 13,
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0, padding: "16px 16px" }}>
                      ...
                      <div
                        style={{
                          fontSize: 11,
                          color: "var(--text-muted)",
                          marginBottom: !eligible && c.minOrder > 0 ? 6 : 0,
                        }}
                      >
                        {c.minOrder > 0
                    ...
                    <div
                      style={{
                        ...
                        width: 0,
                        flexShrink: 0,
                        borderLeft: `1.5px dashed ${eligible ? "var(--border-gold)" : "var(--border)"}`,
                      }}
                    >
                      <span
                        style={{
...
export default function Checkout() {
  ...
  const [defaultAddrToApply, setDefaultAddrToApply] = useState(null);
  ...
  const applyAddress = (addr) => {
    ...
    setShip((f) => ({
      ...
      provinceCode: matchedProvince ? matchedProvince.code : f.provinceCode,
      wardName: addr.ward,
    ...
    (async () => {
      let lat = null,
        lng = null;
      try {
        const query = `${addr.ward}, ${addr.province}, Vietnam`;
  ...
  // Lấy danh sách địa chỉ đã lưu trong profile của tài khoản đang đăng nhập.
  useEffect(() => {
    addressService
      .getAll()
      .then(({ data }) => {
        ...
        if (def) {
          setDefaultAddrToApply(def);
        } else {
          // Chưa có địa chỉ nào -> mở luôn form thêm địa chỉ, không cần bấm nút
          // và không cần hiển thị dòng "Chưa có địa chỉ nào được lưu".
          setShowAddForm(true);
        }
      })
      .catch(() => {})
  ...
  const selectNewAddrProvince = (opt) => {
    setNewAddrForm((f) => ({
      ...f,
  ...
  // Đơn TOÀN sách điện tử: không giao hàng, không hiện địa chỉ, chỉ thanh toán online.
  ...
  const applyCoupon = async (codeOverride) => {
    ...
    const key = (override ?? couponInput).trim().toUpperCase();
    if (!key) return toast.error("Vui lòng nhập mã giảm giá");
    setCouponLoading(true);
    if (override) setApplyingVoucherCode(key);
    try {
                        ...
                        >
                          ...
                          >
                            <LocationCombobox
                              ...
                              value={newAddrForm.provinceName}
                              options={provinces}
                              loading={provincesLoading}
                              onSelect={selectNewAddrProvince}
                            />
                            <LocationCombobox
                              label="Phường/Xã"
                              placeholder="Gõ để tìm phường/xã"
                              disabledHint="Chọn Tỉnh/Thành phố trước"
                              value={newAddrForm.wardName}
                              options={newAddrWards}
                              loading={newAddrWardsLoading}
                              disabled={!newAddrForm.provinceCode}
                              onSelect={selectNewAddrWard}
                            />
                          </div>

                          {/* Chưa có địa chỉ nào -> địa chỉ này chắc chắn sẽ
                              là mặc định (backend tự set), không cần hỏi. */}
                          {savedAddresses.length > 0 && (
                            <label
                              style={{
                                display: "flex",
