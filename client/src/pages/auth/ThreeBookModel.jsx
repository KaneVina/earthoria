...
export default function ThreeBookModel() {
  ...
  useEffect(() => {
    ...
    const sunGlowTex = makeGlowTexture(
      "rgba(255,235,170,1)",
      "rgba(245,195,77,0.55)",
    );
    ...
    const sunSprite2 = new THREE.Sprite(
      new THREE.SpriteMaterial({
        map: sunGlowTex,
        transparent: true,
        opacity: 0.45,
    ...
    const planets = planetDefs.map((def) => {
      ...
      const mesh = new THREE.Mesh(
        new THREE.SphereGeometry(def.size, 40, 40),
        mat,
      );
    ...
    const animate = () => {
      ...
      sunSprite2.scale.set(
        5.2 * (1 + Math.sin(t * 1.0 + 1) * 0.05),
        5.2 * (1 + Math.sin(t * 1.0 + 1) * 0.05),
        1,
      );
