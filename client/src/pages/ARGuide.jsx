...
const STEPS = [
  {
    ...
    step: '01',
    title: 'Tải Ứng Dụng',
    subtitle: 'Earthoria AR',
    desc: 'Tải miễn phí trên App Store hoặc Google Play. Ứng dụng nhẹ, cài trong 30 giây, không cần đăng ký trước.',
    detail: 'Yêu cầu iOS 14+ hoặc Android 10+, RAM tối thiểu 3GB.',
    icon: 'download',
    color: '#4a9e3f',
    bg: '#0d3330',
  },
  {
    ...
    desc: 'Mở cuốn sách Earthoria. Các trang có biểu tượng ◉ màu xanh là trang AR — nơi thế giới 3D sẽ hiện ra.',
  ...
  },
  {
    ...
    desc: 'Mở app, chọn "Quét AR", hướng camera vào trang sách cách 20–40cm. Hệ thống nhận diện trong 1–2 giây.',
...
const FEATURES = [
  { icon: '🦁', title: '120+ Mô Hình AR', desc: 'Mỗi cuốn sách có hơn 120 mô hình 3D siêu thực được nghiên cứu từ dữ liệu khoa học.' },
  { icon: '🎙️', title: 'AI Tutor Thông Minh', desc: 'Trợ lý AI đóng vai nhà tự nhiên học, trả lời hàng trăm câu hỏi theo cách phù hợp với trẻ.' },
  { icon: '🔊', title: 'Âm Thanh 3D Vòm', desc: 'Công nghệ spatial audio 360° tái hiện âm thanh sinh thái chân thực từng ngách rừng.' },
  { icon: '📶', title: 'Offline 100%', desc: 'Sau lần tải đầu, toàn bộ AR hoạt động không cần internet. Chỉ AI Tutor cần kết nối.' },
]
    ...
    <div id="ar-guide-main" style={{ background: '#050e0a', minHeight: '100vh', color: '#faf8f3', fontFamily: "'Be Vietnam Pro', sans-serif", overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 3, maxWidth: '1400px', margin: '0 auto', padding: '140px 100px 80px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
            <button
              ...
              style={{ background: '#4a9e3f', color: '#0a0e0c', border: 'none', padding: '16px 40px', fontFamily: "'Be Vietnam Pro', sans-serif", fontSize: '11px', letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', fontWeight: 400, transition: 'all 0.3s' }}
              onMouseEnter={e => e.currentTarget.style.background = '#5cb84f'}
              onMouseLeave={e => e.currentTarget.style.background = '#4a9e3f'}
            >
              Bắt Đầu Ngay
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <div style={{ width: '36px', height: '36px', border: '0.5px solid rgba(74,158,63,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4a9e3f" strokeWidth="1.5"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z"/><path d="M10 8l6 4-6 4V8z" fill="#4a9e3f"/></svg>
              </div>
              <span style={{ fontSize: '12px', color: 'rgba(250,248,243,0.45)' }}>Xem video demo 2 phút</span>
            </div>
          </div>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', alignItems: 'center', height: '60px', gap: '0' }}>
          {STEPS.map((step, i) => (
            ...
            >
              ...
              <span style={{ display: 'none', ['@media (min-width: 900px)']: { display: 'block' } }}>{step.title}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        >
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '120px 100px', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center', position: 'relative', zIndex: 1 }}>
            ...
            <div>
              ...
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(22px, 2.5vw, 32px)', fontWeight: 300, fontStyle: 'italic', color: step.color, marginBottom: '32px' }}>{step.subtitle}</h3>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', background: 'rgba(74,158,63,0.08)', border: '0.5px solid rgba(74,158,63,0.2)', maxWidth: '440px' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={step.color} strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '48px' }}>
                {idx > 0 && (
                  ...
                  >
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
                    Trước
                  </button>
                )}
                {idx < STEPS.length - 1 && (
      ...
      <section style={{ background: '#071210', padding: '120px 100px', borderTop: '0.5px solid rgba(74,158,63,0.1)' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1px', background: 'rgba(255,255,255,0.06)' }}>
            {FEATURES.map((f, i) => (
              <div key={i}
                style={{ background: '#071210', padding: '52px 48px', transition: 'background 0.4s', cursor: 'default' }}
                onMouseEnter={e => e.currentTarget.style.background = '#0d1f1a'}
                onMouseLeave={e => e.currentTarget.style.background = '#071210'}
              >
      ...
      <section style={{ background: '#050e0a', padding: '120px 100px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          ].map(([q, a], i) => <FAQItem key={i} q={q} a={a} color="#4a9e3f" />)}
        </div>
      </section>
      ...
      <section style={{ background: '#0d3330', padding: '140px 100px', textAlign: 'center', position: 'relative', overflow: 'hidden', borderTop: '0.5px solid rgba(74,158,63,0.15)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/shop">
              <button style={{ background: '#4a9e3f', color: '#0a0e0c', border: 'none', padding: '18px 48px', fontFamily: "'Be Vietnam Pro', sans-serif", fontSize: '11px', letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 400, transition: 'all 0.3s' }}
                onMouseEnter={e => e.currentTarget.style.background = '#5cb84f'}
                onMouseLeave={e => e.currentTarget.style.background = '#4a9e3f'}
              >
                Mua Sách Ngay
...
function DownloadDemo({ color }) {
  ...
  return (
    <div style={{ width: '320px' }}>
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', padding: '40px 32px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          {[['🍎', 'App Store', 'iOS 14+'], ['🤖', 'Google Play', 'Android 10+']].map(([icon, label, sub], i) => (
            <button key={i}
        ...
        {platform !== null && (
          <div style={{ marginTop: '20px', padding: '12px', background: 'rgba(74,158,63,0.1)', border: '0.5px solid rgba(74,158,63,0.3)', animation: 'fadeInUp 0.4s ease' }}>
      ...
      <div style={{ marginTop: '16px', padding: '14px 20px', background: 'rgba(74,158,63,0.06)', border: '0.5px solid rgba(74,158,63,0.15)', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
...
function BookDemo({ color }) {
  ...
  return (
    <div style={{ width: '340px', position: 'relative' }}>
      <div style={{ background: '#faf8f3', boxShadow: '0 40px 80px rgba(0,0,0,0.5)', transform: 'perspective(600px) rotateY(-8deg)', transformOrigin: 'left center' }}>
        {/* Book spine */}
        ...
        <div style={{ padding: '28px 28px 28px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
            {pages.map((hasAR, i) => (
              ...
              >
                {hasAR && (
                  <span style={{ fontSize: '16px', color: hoveredPage === i ? '#fff' : color, fontWeight: 500, animation: hoveredPage === i ? 'pulse 1s ease-in-out infinite' : 'none' }}>◉</span>
                )}
                {hasAR && hoveredPage === i && (
    ...
    <div style={{ width: '280px' }}>
      ...
      <div style={{ background: '#0a0e0c', borderRadius: '24px', border: '6px solid rgba(255,255,255,0.1)', overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.6)' }}>
        <div style={{ padding: '16px', background: 'linear-gradient(160deg, #0f2e20 0%, #1a4a30 100%)', minHeight: '460px', display: 'flex', flexDirection: 'column' }}>
          {/* Status bar */}
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginBottom: '16px', paddingTop: '4px' }}>
            <span>9:41</span><span>●●●</span>
          </div>
          {/* AR Label */}
          <div style={{ display: 'inline-block', background: 'rgba(74,158,63,0.15)', border: '0.5px solid rgba(74,158,63,0.4)', padding: '4px 10px', fontSize: '9px', letterSpacing: '0.14em', textTransform: 'uppercase', color: color, marginBottom: '12px', width: 'fit-content' }}>
            {scanning ? '⟳ Đang quét...' : modelVisible ? '✓ Đã nhận diện' : 'Chế độ AR'}
          </div>
          {/* Viewfinder */}
          <div style={{ flex: 1, border: scanning ? `1px solid ${color}` : '1px dashed rgba(255,255,255,0.15)', borderRadius: '12px', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', marginBottom: '12px', minHeight: '260px', background: modelVisible ? 'rgba(74,158,63,0.05)' : 'transparent', transition: 'all 0.5s' }}>
            ...
            {modelVisible ? (
              <div style={{ textAlign: 'center', animation: 'fadeInUp 0.6s ease' }}>
                <div style={{ fontSize: '52px', animation: 'modelFloat 3s ease-in-out infinite', display: 'block' }}>🦁</div>
                <div style={{ fontSize: '11px', color: color, marginTop: '8px', letterSpacing: '0.1em' }}>Panthera leo</div>
          ...
          <button
            ...
            style={{ background: scanning ? 'rgba(74,158,63,0.3)' : modelVisible ? 'rgba(74,158,63,0.5)' : color, border: 'none', borderRadius: '50px', height: '44px', cursor: scanning || modelVisible ? 'not-allowed' : 'pointer', fontSize: '11px', color: modelVisible ? color : '#0a0e0c', letterSpacing: '0.1em', fontFamily: "'Be Vietnam Pro', sans-serif", transition: 'all 0.3s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
          >
            {modelVisible ? '✓ Đã quét thành công' : scanning ? '⟳ Đang quét...' : '◉ Bắt đầu quét AR'}
          </button>
        </div>
      </div>
    </div>
...
function ModelDemo({ color, modelVisible }) {
  ...
  const HOTSPOTS = [
    { x: '55%', y: '30%', label: 'Bờm', desc: 'Bảo vệ cổ trong đấu tranh', top: true },
  ...
  return (
    <div style={{ width: '340px' }}>
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '4px', overflow: 'hidden' }}>
        {/* 3D viewport */}
        <div
          style={{ height: '300px', background: 'radial-gradient(ellipse at center, rgba(74,158,63,0.05) 0%, transparent 70%)', position: 'relative', cursor: dragging ? 'grabbing' : 'grab', userSelect: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          onMouseDown={e => { setDragging(true); setLastPos({ x: e.clientX, y: e.clientY }) }}
          onMouseMove={e => { if (!dragging) return; setRotation(r => ({ x: r.x + (e.clientY - lastPos.y) * 0.5, y: r.y + (e.clientX - lastPos.x) * 0.5 })); setLastPos({ x: e.clientX, y: e.clientY }) }}
        ...
        <div style={{ padding: '12px 16px', borderTop: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {['−', '⟳', '+'].map((btn, i) => (
              <button key={i} onClick={() => { if (i === 0) setZoom(z => Math.max(0.5, z - 0.2)); if (i === 1) { setZoom(1); setRotation({ x: 0, y: 0 }) } if (i === 2) setZoom(z => Math.min(2.5, z + 0.2)) }}
