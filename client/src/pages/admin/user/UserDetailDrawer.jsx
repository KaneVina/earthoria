...
function InfoRow({ label, value }) {
  return (
    ...
    >
      ...
      <span
        style={{ color: "var(--a-ink)", fontWeight: 500, textAlign: "right" }}
      >
        {value}
      </span>
    </div>
...
export default function UserDetailDrawer({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get(`/admin/users/${userId}/detail`).then((r) => r.data.data),
    enabled: !!userId,
  });
  ...
  const roleCfg = data
    ? (ROLE_CONFIG[data.role] ?? ROLE_CONFIG.CUSTOMER)
    : null;
          ...
          <div style={{ padding: 24, flex: 1 }}>
            ...
            >
              {data.avatar ? (
                ...
                <div
                  ...
                  style={{
                    ...
                    height: 56,
                    fontSize: 20,
                    background: avatarColor(data.name),
                    flexShrink: 0,
                  }}
                >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  ...
                  <span
                    className={`a-badge ${data.isActive ? "success" : "danger"}`}
                  >
              ...
              >
                ...
                {showToggle && (
                  <button
                    ...
                    onClick={() =>
                      onToggleLock?.(data, data.isActive ? "lock" : "unlock")
                    }
                  >
            ...
            {!data.isActive && data.lockReason && (
              ...
              >
                ...
                <div>
                  ...
                  >
                    Lý do khóa{" "}
                    {data.lockedAt && `· ${formatDate(data.lockedAt)}`}
                  </div>
                  <div
                    style={{ fontSize: 12, color: "#7a4440", lineHeight: 1.6 }}
                  >
                    {data.lockReason}
                  </div>
                </div>
                ...
                >
                  <div
                    style={{
                      ...
                      alignItems: "center",
                      gap: 12,
                      marginBottom: loyalty.isMaxTier ? 0 : 12,
                    }}
                  >
                    {loyalty.tier.image ? (
                      <img
                        src={loyalty.tier.image}
                        alt={loyalty.tier.name}
                        style={{
                          width: 44,
                          height: 44,
                          objectFit: "contain",
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div
                        style={{ fontSize: 10, color: "rgba(13,51,48,0.4)" }}
                      >
                        Đã chi tiêu
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>
                        {formatPrice(loyalty.spend ?? 0)}
                      </div>
                    </div>
                  </div>
                      ...
                      >
                        <div
                          style={{
                            ...
                            background: loyalty.tier.color || "var(--a-green)",
                            borderRadius: 4,
                          }}
                        />
                      </div>
                      <div
                        style={{ fontSize: 11, color: "rgba(13,51,48,0.55)" }}
                      >
                        Chi thêm{" "}
                        <strong>{formatPrice(loyalty.amountToNext)}</strong> để
                        lên{" "}
                        <strong>
                          Hạng {loyalty.nextTier.roman} ·{" "}
                  ...
                  {data.children.map((child) => (
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 5 }}>
                        {child.isLocked && (
                          <span className="a-badge danger">Khóa AR</span>
                        )}
                        <span
                          className={`a-badge ${child.isActive ? "success" : "neutral"}`}
                        >
            ...
            <div>
              <SectionTitle icon={ShoppingBag}>
                Đơn hàng ({data._count?.orders ?? 0}) · Đã mua{" "}
