...
export default function Users() {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, email, reason }) =>
      api.put(`/admin/users/${id}/toggle`, { email, reason }),
    onSuccess: () => {
      toast.success(
  ...
  const promoteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "Cập nhật cấp bậc thành công! Email thông báo đã được gửi.",
      );
      qc.invalidateQueries(["admin-users"]);
      setPromoteUser(null);
    },
  ...
  const canPromote = (targetRole) =>
    ["CUSTOMER", "DEALER"].includes(targetRole);

  const canToggle = (targetUser) => {
    if (targetUser.id === viewer?.id) return false; // không tự khóa chính mình
    if (viewerRole === "STAFF")
      return ["CUSTOMER", "DEALER"].includes(targetUser.role);
    if (viewerRole === "ADMIN")
      return ["CUSTOMER", "DEALER", "STAFF"].includes(targetUser.role);
    return false;
  };
  ...
  const lockEmailMatches =
    confirmUser?.action === "lock"
      ? lockEmailInput.trim().toLowerCase() ===
        confirmUser.user.email.toLowerCase()
      : true;
  const lockReasonValid = lockReason.trim().length >= 10;
  const canSubmitLock =
    confirmUser?.action === "unlock" || (lockEmailMatches && lockReasonValid);
  ...
  return (
    <AdminLayout>
      ...
      <div className="a-page-header">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 12, color: "rgba(13,51,48,0.4)" }}>
            Tổng <strong style={{ color: "var(--a-ink)" }}>{total}</strong> tài
            khoản
          </div>
          <button
      ...
      {selectedIds.size > 0 && (
        ...
        >
          <div
            style={{ fontSize: 12.5, color: "var(--a-ink)", fontWeight: 500 }}
          >
              ...
              <tr>
                <th style={{ width: 34 }}>
                  <input
                    ...
                    style={{
                      cursor: selectableUsers.length ? "pointer" : "default",
                    }}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} style={{ padding: 56, textAlign: "center" }}>
                    <UsersIcon
                      size={30}
                      style={{ color: "rgba(13,51,48,0.15)", marginBottom: 10 }}
                    />
                ...
                users.map((user) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={`a-badge ${user.isActive ? "success" : "danger"}`}
                        >
          ...
          <div className="a-modal" style={{ maxWidth: 440 }}>
            ...
            <div className="a-modal-body">
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    height: 40,
                    fontSize: 16,
                    background: avatarColor(confirmUser.user.name),
                  }}
                >
              ...
              {confirmUser.action === "unlock" ? (
                <p
                  style={{
                    fontSize: 13,
                    color: "rgba(13,51,48,0.65)",
                    lineHeight: 1.6,
                  }}
                >
                  Tài khoản này sẽ được mở khóa. Người dùng có thể đăng nhập và
                  mua hàng bình thường. Email thông báo sẽ được gửi tự động.
                </p>
              ) : (
                <>
                  <p
                  ...
                  <div
                    style={{
                      fontSize: 11,
                      color:
                        lockReason.length > 0 && !lockReasonValid
                          ? "#c05050"
                          : "rgba(13,51,48,0.4)",
                      marginTop: 5,
                    }}
            ...
            <div className="a-modal-footer">
              <button
                ...
                style={{
                  background:
                    confirmUser.action === "lock" ? "#c05050" : "#4a9e3f",
                }}
                onClick={() =>
              ...
              <button className="a-btn-ghost" onClick={closeLockModal}>
                Hủy
              </button>
            </div>
          </div>
          ...
          <div className="a-modal" style={{ maxWidth: 440 }}>
            ...
            <div className="a-modal-body">
              ...
              >
                {selectedUsers
                  .filter((u) =>
                    bulkAction.action === "lock" ? u.isActive : !u.isActive,
                  )
                  .map((u) => (
                    <span
                      key={u.id}
              ...
              {bulkAction.action === "unlock" ? (
                ...
                >
                  Các tài khoản trên sẽ được mở khóa. Email thông báo sẽ được
                  gửi tự động cho từng người.
                </p>
              ) : (
                <>
                  <p
            ...
            <div className="a-modal-footer">
              <button
                ...
                style={{
                  background:
                    bulkAction.action === "lock" ? "#c05050" : "#4a9e3f",
                }}
                onClick={() =>
        ...
        >
          <div className="a-modal" style={{ maxWidth: 420 }}>
            ...
            <div className="a-modal-body">
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    height: 40,
                    fontSize: 16,
                    background: avatarColor(promoteUser.name),
                  }}
                >
              ...
              >
                Hệ thống sẽ tự sinh <strong>mã ETR mới</strong> và{" "}
                <strong>mật khẩu tạm thời mới</strong>, gửi email thông báo đến
                người dùng. Họ cần đổi mật khẩu ngay khi đăng nhập lần đầu.
              </p>
            </div>
            <div className="a-modal-footer">
              <button
                ...
                onClick={() =>
                  promoteMutation.mutate({
                    id: promoteUser.id,
                    role:
                      promoteUser.role === "CUSTOMER" ? "DEALER" : "CUSTOMER",
                  })
                }
              >
