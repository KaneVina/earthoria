// Tickets.jsx — Quản lý yêu cầu liên hệ (form /contact) cho Staff & Admin
import { useState } from 'react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import {
  X, Search, Send, UserCheck, UserX, Mail, Phone, Building2,
  Clock, CheckCheck,
} from 'lucide-react'
...
const CONTACT_METHOD_LABEL = {
  phone: 'Điện thoại', zalo: 'Zalo', email: 'Email', facebook: 'Facebook',
}
...
function TicketDrawer({ ticket, onClose, currentUser }) {
  ...
  const { data: detail, isLoading } = useQuery({
    ...
    queryFn: () => ticketService.getTicketById(ticket.id).then(r => r.data.data),
    enabled: !!ticket,
  })
  ...
  const replyMutation = useMutation({
    ...
    onError: (err) => toast.error(err.response?.data?.message || 'Gửi phản hồi thất bại'),
  })
  ...
  return (
    ...
    >
      ...
      <div style={{
        position: 'relative', zIndex: 1, width: 460, maxWidth: '100%', background: '#fff',
        boxShadow: '-8px 0 40px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', overflowY: 'auto',
      }}>
        {/* Header */}
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontFamily: 'monospace', fontSize: 14, color: 'var(--a-ink)', fontWeight: 600 }}>
              {t.code}
            </div>
          </div>
        ...
        <div style={{ padding: 24, flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            <span className="a-badge neutral">{TICKET_SUBJECT[t.subject] || t.subject}</span>
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <div>
                ...
                {t.user && <div style={{ fontSize: 11, color: 'rgba(13,51,48,0.45)' }}>Tài khoản: {t.user.email}</div>}
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, color: 'var(--a-ink)' }}>
              ...
              {t.phone && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <Phone size={13} strokeWidth={1.5} color="rgba(13,51,48,0.4)" />
                  <a href={`tel:${t.phone}`}>{t.phone}</a>
                </div>
              )}
              {t.company && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <Building2 size={13} strokeWidth={1.5} color="rgba(13,51,48,0.4)" />
                  {t.company}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            {t.contactMethods?.length > 0 && (
              <div style={{ marginTop: 10, fontSize: 11, color: 'rgba(13,51,48,0.5)' }}>
                Muốn được liên hệ qua:{' '}
                <strong style={{ color: 'var(--a-ink)' }}>
                  {t.contactMethods.map(m => CONTACT_METHOD_LABEL[m] || m).join(', ')}
                </strong>
              </div>
            )}
          </div>
          ...
          {t.extraFields && Object.keys(t.extraFields).length > 0 && (
            <div style={{ marginBottom: 20, background: 'var(--a-surface)', borderRadius: 8, padding: '13px 16px' }}>
              <div style={{ fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(13,51,48,0.38)', marginBottom: 8, fontWeight: 500 }}>
                Thông tin thêm
              </div>
              {Object.entries(t.extraFields).filter(([, v]) => v).map(([k, v]) => (
                <div key={k} style={{ fontSize: 12, marginBottom: 4 }}>
                  <span style={{ color: 'rgba(13,51,48,0.5)' }}>{EXTRA_FIELD_LABEL[k] || k}: </span>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--a-ink)', whiteSpace: 'pre-wrap' }}>
              {t.message}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            <div style={{ fontSize: 12, color: 'rgba(13,51,48,0.55)' }}>
              {t.assignedTo
                ? <>Phụ trách: <strong style={{ color: 'var(--a-ink)' }}>{t.assignedTo.name}</strong></>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {isLoading ? (
              <div style={{ fontSize: 12, color: 'rgba(13,51,48,0.4)' }}>Đang tải...</div>
            ) : !t.replies?.length ? (
              <div style={{ fontSize: 12, color: 'rgba(13,51,48,0.4)' }}>Chưa có phản hồi nào</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {t.replies.map((r) => (
                  <div key={r.id} style={{ background: 'var(--a-surface)', borderRadius: 8, padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--a-ink)' }}>{r.staff?.name}</span>
                      <span style={{ fontSize: 11, color: 'rgba(13,51,48,0.4)' }}>{formatDateTime(r.createdAt)}</span>
                    </div>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              >
                ...
                {Object.entries(TICKET_STATUS).map(([key, label]) => (
                  <option key={key} value={key}>Chuyển sang: {label}</option>
                ))}
              </select>
              <button
                type="submit"
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(13,51,48,0.08)' }}>
            <div style={{ fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(13,51,48,0.38)', marginBottom: 8, fontWeight: 500 }}>
...
export default function Tickets() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => ticketService.getTickets({ page, limit: 15, status, search: search || undefined }).then(r => r.data.data),
    ...
    <AdminLayout>
      ...
      <div className="a-page-header">
        <div>
          ...
          <h1 className="a-page-title">Yêu Cầu <em>Liên Hệ</em></h1>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(13,51,48,0.4)' }}>
          Tổng <strong style={{ color: 'var(--a-ink)' }}>{total}</strong> yêu cầu
        </div>
      </div>

      {/* Search */}
      <div style={{ marginBottom: 14, position: 'relative', maxWidth: 320 }}>
        <Search size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'rgba(13,51,48,0.35)' }} />
        <input
          className="a-input"
      ...
      <div className="a-table-card">
        <div className="a-table-wrap">
          <table className="a-table">
            <thead>
              <tr>
                {['Mã yêu cầu', 'Người gửi', 'Chủ đề', 'Trạng thái', 'Phụ trách', 'Ngày gửi'].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(t)}>
                  ...
                  <td className="a-td-muted">{formatDateTime(t.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Detail drawer */}
      <TicketDrawer ticket={selected} onClose={() => setSelected(null)} currentUser={user} />
    </AdminLayout>
  )
}