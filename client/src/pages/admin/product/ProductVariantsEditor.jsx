...
import { FORMAT_LABEL, computeModePrice, emptyVariant } from "./productFormUtils";
...
export default function ProductVariantsEditor({ variants, setVariants, onDeleteVariant }) {
  ...
  const addVariant = (format) => setVariants((prev) => [...prev, emptyVariant(format)]);
      ...
      {availableFormats.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          {availableFormats.map((f) => (
            <button key={f} type="button" className="a-btn-ghost" onClick={() => addVariant(f)}>
  ...
  return (
    <div style={{ border: "1px solid #e8e5de", borderRadius: 12, padding: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {!isNew && (
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "rgba(13,51,48,0.6)" }}>
              <input type="checkbox" checked={variant.isActive} onChange={(e) => onChange({ isActive: e.target.checked })} />
              Đang bán
            </label>
          )}
          {canRemove && (
            <button type="button" className="a-btn-icon delete" title="Xóa định dạng này" onClick={onRemove}>
              <Trash2 size={12} />
            </button>
          )}
        </div>
      ...
      <div className="a-form-grid">
        <div className="a-form-group">
          ...
          <input className="a-input" type="number" min={0} value={variant.price} onChange={(e) => onChange({ price: e.target.value })} placeholder="420000" required />
        </div>

        <div className="a-form-group">
          <label className="a-form-label">Đơn vị</label>
          <input className="a-input" value={variant.unit} onChange={(e) => onChange({ unit: e.target.value })} placeholder="Cuốn / Bản..." />
        </div>
        ...
        {!isNew && (
          <div className="a-form-group">
            ...
            <div style={{ fontSize: 13, fontWeight: 600, padding: "8px 0" }}>{variant.sold ?? 0}</div>
          </div>
        )}

        <div className="a-form-group">
          <label className="a-form-label">Giá bán khách hàng</label>
          <ModeToggle mode={variant.saleMode} onChange={(m) => onChange({ saleMode: m })} />
          {variant.saleMode === "direct" ? (
            <input className="a-input" type="number" min={0} value={variant.salePrice} onChange={(e) => onChange({ salePrice: e.target.value })} placeholder="260000" />
          ) : (
            <>
              <input className="a-input" type="number" min={0} max={100} value={variant.salePercent} onChange={(e) => onChange({ salePercent: e.target.value })} placeholder="% giảm, vd 38" />
              <span style={{ fontSize: 10, color: "rgba(13,51,48,0.5)" }}>
                Giá sau giảm: {formatPrice(computeModePrice("percent", variant.salePercent, "", variant.price) ?? 0)}
              </span>
            </>
          )}
        </div>
        ...
        <div className="a-form-group">
          ...
          <ModeToggle mode={variant.dealerMode} onChange={(m) => onChange({ dealerMode: m })} />
          {variant.dealerMode === "direct" ? (
            <input className="a-input" type="number" min={0} value={variant.dealerPrice} onChange={(e) => onChange({ dealerPrice: e.target.value })} placeholder="200000" />
          ) : (
            <>
              <input className="a-input" type="number" min={0} max={100} value={variant.dealerPercent} onChange={(e) => onChange({ dealerPercent: e.target.value })} placeholder="% chiết khấu, vd 50" />
              <span style={{ fontSize: 10, color: "rgba(13,51,48,0.5)" }}>
                Giá đại lý: {formatPrice(computeModePrice("percent", variant.dealerPercent, "", variant.price) ?? 0)}
              </span>
            </>
          )}
        </div>
...
function VariantCodePreview({ variant }) {
  if (variant.id) {
    return (
      <span style={{ fontFamily: "monospace", fontSize: 11, background: "#f5f3ee", padding: "2px 8px", borderRadius: 5, color: "#0D3330" }}>
        {variant.productCode ?? "—"}
      </span>
    );
  }
  return (
    <span
      style={{ fontFamily: "monospace", fontSize: 11, color: "rgba(13,51,48,0.4)" }}
