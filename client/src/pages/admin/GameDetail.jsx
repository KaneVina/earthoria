...
function getLitRoles(accessType) {
  if (accessType === "PUBLIC") return ["GUEST", "CUSTOMER", "DEALER", "STAFF", "ADMIN"];
...
export default function GameDetail() {
  ...
  const { data: preselectBook } = useQuery({
    ...
    queryFn: () => api.get(`/admin/products/${preselectBookId}`).then((r) => r.data.data),
  ...
  const { data: bookSuggestions = [] } = useQuery({
    ...
    queryFn: () => api.get("/admin/products/search", { params: { q: bookQuery } }).then((r) => r.data.data),
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => toast.error(e.response?.data?.message || "Tạo trò chơi thất bại!"),
  });
  ...
  const qrUrl = isEditMode && selectedBook && game ? `${window.location.origin}/game/${selectedBook.slug}/${game.code}` : "";

  const handleDownloadQr = () => {
    const canvas = qrWrapRef.current?.querySelector("canvas");
    if (!canvas) return;
    const safeName = (game?.title || "game").trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
  ...
  if (isEditMode && loadingGame) {
    return (
      <AdminLayout crumbs={[{ label: "Trò chơi" }, { label: "Chi tiết" }]}>
        <div style={{ padding: 60, textAlign: "center", color: "rgba(13,51,48,0.3)" }}>Đang tải...</div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout crumbs={[{ label: "Trò chơi" }, { label: isEditMode ? "Chỉnh sửa" : "Tạo mới" }]}>
      <div className="a-page-header" style={{ marginBottom: 20 }}>
        <div>
          <button type="button" className="a-btn-ghost" style={{ marginBottom: 12 }} onClick={() => navigate("/dashboard/games")}>
              ...
              {showSuggest && bookQuery.trim().length >= 1 && (
                ...
                >
                  {bookSuggestions.length === 0 ? (
                    <div style={{ padding: 14, fontSize: 12, color: "rgba(13,51,48,0.4)" }}>Không tìm thấy sách</div>
                  ) : (
                    bookSuggestions.map((b) => (
                      ...
                      >
                        <div className="a-book-thumb" style={{ width: 24, height: 32 }}>
            ...
            <>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                  <div className="a-book-thumb" style={{ width: 34, height: 46 }}>
                    {selectedBook.coverImage ? <img src={selectedBook.coverImage} alt={selectedBook.title} /> : <Upload size={14} />}
                  </div>
                  <div style={{ fontWeight: 600, fontSize: 13 }}>{selectedBook.title}</div>
                </div>
                <button type="button" className="a-btn-ghost" style={{ fontSize: 11, padding: "6px 10px", flexShrink: 0 }} onClick={() => setSelectedBook(null)}>
                  Đổi sách
                </button>
              </div>
          ...
          {selectedBook && (isEditMode || gameType) && (
            <>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    {def && (
                      <div className="a-td-muted" style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                        <def.icon size={11} /> {def.label}
                      </div>
                    )}
                    {isEditMode && game && (
                      <span className={`a-badge ${game.isActive ? "success" : "neutral"}`} style={{ marginTop: 4 }}>
                ...
                {!isEditMode && (
                  <button type="button" className="a-btn-ghost" style={{ fontSize: 11, padding: "6px 10px", flexShrink: 0 }} onClick={() => setGameType(null)}>
                    Đổi loại
                  </button>
                )}
              </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="a-form-group" style={{ marginBottom: 12 }}>
                  <label className="a-form-label">Hướng dẫn chơi (tuỳ chọn)</label>
                  <input
                    className="a-input"
                ...
                <div className="a-form-group" style={{ marginBottom: 16 }}>
                  ...
                  {Editor && form.config !== null && (
                    <Editor config={form.config} onChange={(config) => setForm((f) => ({ ...f, config }))} gameId={id} />
                  )}
                </div>
                  ...
                  <div className="a-access-row">
                    <div className="a-access-toggle">
                      {ACCESS_OPTIONS.map((opt) => (
                        <button
                          ...
                          onClick={() => setForm((f) => ({ ...f, accessType: opt.value }))}
                        >
                    ...
                    <div className="a-role-matrix">
                      {ROLE_MATRIX.map((role) => {
                        ...
                        return (
                          <div key={role.key} className={`a-role-chip ${isLit ? "lit" : "dim"}`}>
                            <div className="a-role-chip-icon">
                              <Icon size={13} />
                            </div>
                            <div className="a-role-chip-label">{role.label}</div>
                          </div>
                        );
                      })}
                      <div className="a-role-matrix-hint">Staff và Admin luôn xem được bất kể lựa chọn ở đây</div>
                    </div>
                  </div>
                </div>

                {isEditMode && game && (
                  <div style={{ fontSize: 11, color: "rgba(13,51,48,0.5)", marginBottom: 16 }}>
                    {game.playCount} lượt chơi hoàn thành
                  </div>
                )}

                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button type="submit" className="a-btn-primary" style={{ flex: 1, justifyContent: "center" }} disabled={isSaving}>
                    {isSaving ? "Đang lưu..." : isEditMode ? "Lưu thay đổi" : "Tạo trò chơi"}
                  </button>

                  {isEditMode && game && (
                    <button type="button" className="a-btn-ghost" onClick={() => toggleMutation.mutate()} disabled={toggleMutation.isPending}>
                      {game.isActive ? (
                        <>
                  ...
                  {isEditMode && game && (
                    <button
                      ...
                      style={{ color: "#e34948" }}
                      onClick={() => {
                        if (window.confirm("Xóa vĩnh viễn trò chơi này? Không thể hoàn tác.")) {
          ...
          {isEditMode && game && (
            <div className="a-chart-card" style={{ marginTop: 16 }}>
              ...
              {leaderboard.length === 0 ? (
                <div style={{ padding: "20px 0", textAlign: "center", color: "rgba(13,51,48,0.3)", fontSize: 12 }}>
                ...
                <div className="g-leaderboard">
                  {leaderboard.slice(0, 5).map((r, i) => (
                    <div className="g-leaderboard-row" key={r.id}>
                      ...
                      <span className="g-leaderboard-name">{r.child?.name || r.user?.name || "Ẩn danh"}</span>
