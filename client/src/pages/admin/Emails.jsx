// Emails.jsx — Quản lý Email (lịch sử gửi qua Resend + soạn gửi thủ công)
// Đặt tại: src/pages/admin/Emails.jsx
...
function ComposeModal({ onClose, onSent }) {
  ...
  const pickSuggestion = (email) => {
    const segments = form.to.split(',').map(s => s.trim()).filter(Boolean)
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          ...
          <div className="a-modal-footer">
            <button type="button" className="a-btn-ghost" onClick={onClose}>Hủy</button>
            <button type="submit" className="a-btn-primary" disabled={sendMutation.isPending}>
              <Send size={13} />
    ...
    <div className="a-modal-overlay" onClick={onClose}>
      <div className="a-modal wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="a-modal-body">
          {isLoading ? (
            <div style={{ padding: '30px 0', textAlign: 'center', color: 'var(--a-ink-40)' }}>Đang tải...</div>
          ) : !data ? (
            <div style={{ padding: '30px 0', textAlign: 'center', color: 'var(--a-ink-40)' }}>Không tìm thấy email</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div className="a-form-grid">
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12 }}>{formatDate(data.createdAt)}</div>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: data.html || '<em>Không có nội dung HTML</em>' }}
                />
              </div>
            </div>
          )}
...
export default function Emails() {
  ...
  const { data, isLoading, isFetching, refetch } = useQuery({
    ...
    queryFn: () => api.get('/admin/emails', { params: { limit: 30 } }).then(r => r.data.data),
    staleTime: 30_000,
  })
    ...
    <AdminLayout>
      <div className="a-page-header">
        <div>
          ...
          <h1 className="a-page-title">Quản lý <em>Email</em></h1>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="a-btn-ghost" onClick={() => refetch()} disabled={isFetching}>
            <RefreshCw size={13} style={{ animation: isFetching ? 'spin 1s linear infinite' : 'none' }} />
            Làm mới
          </button>
          <button className="a-btn-primary" onClick={() => setShowCompose(true)}>
      ...
      <div className="a-table-card">
        <div className="a-table-head">
          <h3 className="a-table-title">Lịch sử <em>gửi email</em></h3>
        ...
        <div className="a-table-wrap">
          <table className="a-table">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={5} style={{ padding: 40, textAlign: 'center', color: 'var(--a-ink-40)' }}>
                    Đang tải...
                  </td>
                </tr>
              ) : emails.length === 0 ? (
                ...
                emails.map((email) => (
                  <tr key={email.id}>
                    <td>
                      <div style={{ fontWeight: 500, fontSize: 12 }}>{email.to?.[0]}</div>
                      {email.to?.length > 1 && (
                        <div className="a-td-muted">+{email.to.length - 1} người khác</div>
                      )}
                    </td>
                    <td style={{ maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {email.subject}
                    </td>
                    <td><StatusBadge status={email.status} label={email.statusLabel} /></td>
                    ...
                    <td>
                      <button className="a-btn-icon edit" onClick={() => setDetailId(email.id)} title="Xem chi tiết">
                        <Eye size={13} />
                      </button>
                    </td>
                  </tr>
