...
function Stars({ rating }) {
  return (
    <span style={{ display: 'inline-flex', gap: 1, color: 'var(--a-gold, #b8934a)' }}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star key={i} size={12} fill={i < rating ? 'currentColor' : 'none'} strokeWidth={1.5} />
      ))}
    </span>
  )
}
...
function ReviewDrawer({ review, onClose }) {
  ...
  const { data: detail, isLoading } = useQuery({
    ...
    queryFn: () => bookService.getAdminReviewById(review.id).then(r => r.data.data),
    enabled: !!review,
  })
  ...
  const visibilityMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(res.data?.data?.isVisible ? 'Đã hiện lại đánh giá' : 'Đã ẩn đánh giá')
      qc.invalidateQueries({ queryKey: ['admin-reviews'] })
      qc.invalidateQueries({ queryKey: ['admin-review', review.id] })
    },
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--a-ink)', fontWeight: 600 }}>
              {r.book?.title}
            </div>
          </div>
        ...
        <div style={{ padding: 24, flex: 1 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <div className="a-user-avatar">{r.user?.name?.[0]?.toUpperCase()}</div>
              <div>
                <div style={{ fontWeight: 500, fontSize: 13 }}>{r.user?.name}</div>
                <div style={{ fontSize: 11, color: 'rgba(13,51,48,0.45)' }}>{r.user?.email}</div>
              </div>
            </div>
          ...
          {/* Nội dung đánh giá */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(13,51,48,0.38)', marginBottom: 8, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
              Nội dung <Stars rating={r.rating} />
            </div>
            {r.title && (
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--a-ink)', marginBottom: 6 }}>
                {r.title}
              </div>
            )}
          ...
          {r.reply && (
            <div style={{ marginBottom: 20, background: 'var(--a-surface)', borderRadius: 8, padding: '13px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--a-ink)' }}>
          ...
          <form onSubmit={handleReplySubmit} style={{ marginBottom: 20 }}>
            ...
            >
              <Send size={13} />
              {replyMutation.isPending ? 'Đang gửi...' : r.reply ? 'Cập nhật phản hồi' : 'Gửi phản hồi'}
            </button>
          </form>

          {/* Ẩn / hiện đánh giá */}
          <div style={{ paddingTop: 16, borderTop: '1px solid rgba(13,51,48,0.08)' }}>
            <button
              type="button"
...
export default function Reviews() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => bookService.getAdminReviews({
      page, limit: 15,
    ...
    <AdminLayout>
      ...
      <div className="a-page-header">
        <div>
          ...
          <h1 className="a-page-title">Đánh Giá <em>Sách</em></h1>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(13,51,48,0.4)' }}>
          Tổng <strong style={{ color: 'var(--a-ink)' }}>{total}</strong> đánh giá
        </div>
      </div>
      ...
      <div className="a-pills">
        <button
          ...
          onClick={() => { setHasReply(''); setPage(1) }}
        >
          Tất cả
        </button>
        <button
          className={`a-pill${hasReply === 'false' ? ' active' : ''}`}
          onClick={() => { setHasReply('false'); setPage(1) }}
        >
          Chưa phản hồi
        </button>
        <button
          className={`a-pill${hasReply === 'true' ? ' active' : ''}`}
          onClick={() => { setHasReply('true'); setPage(1) }}
        >
          Đã phản hồi
        </button>
      </div>
      ...
      <div className="a-table-card">
        <div className="a-table-wrap">
          <table className="a-table">
            <thead>
              <tr>
                {['Sách', 'Người đánh giá', 'Sao', 'Nội dung', 'Phản hồi', 'Hiển thị', 'Ngày gửi'].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(r)}>
                  <td>
                    <div style={{ fontWeight: 500, fontSize: 12, maxWidth: 180 }}>{r.book?.title}</div>
                  </td>
                  <td>
                    <div style={{ fontWeight: 500, fontSize: 12 }}>{r.user?.name}</div>
                  ...
                  <td className="a-td-muted">{formatDateTime(r.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
