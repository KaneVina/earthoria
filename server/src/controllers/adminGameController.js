...
const GAME_TYPES = ['MEMORY_MATCH', 'MATCH_PAIRS', 'WORD_SEARCH', 'LETTER_HUNT']


function validateGameConfig(gameType, config) {
  if (!config || typeof config !== 'object') return 'Thiếu dữ liệu cấu hình trò chơi'

  switch (gameType) {
    case 'MEMORY_MATCH': {
      const pairs = config.pairs
      if (!Array.isArray(pairs) || pairs.length < 2) return 'Cần ít nhất 2 cặp thẻ'
    ...
    case 'MATCH_PAIRS': {
      const pairs = config.pairs
      if (!Array.isArray(pairs) || pairs.length < 2) return 'Cần ít nhất 2 cặp để nối'
      for (const p of pairs) {
        if (!p?.id || !String(p?.left || '').trim() || !String(p?.right || '').trim()) {
    ...
    case 'WORD_SEARCH': {
      ...
      if (!Array.isArray(words) || words.length < 1) return 'Cần ít nhất 1 từ để giấu trong bảng'
      if (words.some((w) => !String(w || '').trim())) return 'Có từ đang để trống'
    ...
    case 'LETTER_HUNT': {
      ...
      if (word.replace(/\s/g, '').length > 12) return 'Từ khoá tối đa 12 ký tự (không tính khoảng trắng)'
      return null
    }
    default:
...
exports.createGame = async (req, res) => {
  try {
    const { bookId } = req.params
    const { title, gameType, config, instructions, accessType, thumbnailUrl } = req.body

    if (!title?.trim()) return res.status(400).json({ success: false, message: 'Thiếu tên trò chơi' })
...
exports.updateGame = async (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ success: false, message: 'Không tìm thấy trò chơi' })

    const data = {}
    if (title !== undefined) {
      if (!title?.trim()) return res.status(400).json({ success: false, message: 'Tên trò chơi không được để trống' })
...
exports.updateGameAccess = async (req, res) => {
  try {
    ...
    const game = await prisma.game.update({ where: { id }, data: { accessType } })
    return res.json({ success: true, data: game })
  } catch (err) {
    if (err.code === 'P2025') {
      return res.status(404).json({ success: false, message: 'Không tìm thấy trò chơi' })
