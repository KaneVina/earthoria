...
async function sendCreatePasswordOtp(req, res) {
  try {
    ...
    if (user.password) {
      return formatResponse(res, 400, 'Tài khoản của bạn đã có mật khẩu. Vui lòng dùng chức năng đổi mật khẩu.')
    }
    ...
    try {
      ...
      console.error('[sendCreatePasswordOtp] Failed to send OTP email:', mailErr.message)
      return formatResponse(res, 500, 'Không thể gửi email xác thực. Vui lòng thử lại sau.')
    }

    return formatResponse(res, 200, 'Mã xác thực đã được gửi đến email của bạn.')
  } catch (err) {
...
async function createPassword(req, res) {
  try {
    ...
    if (!user.resetOtpHash || !user.resetOtpExpires) {
      return formatResponse(res, 400, 'Phiên xác thực không hợp lệ. Vui lòng yêu cầu mã mới.')
    }
    ...
    sendPasswordChangedEmail({ to: user.email, name: user.name }).catch((err) =>
      console.error('[createPassword] Failed to send confirmation email:', err.message)
    )

    return formatResponse(res, 200, 'Tạo mật khẩu thành công! Từ giờ bạn có thể đăng nhập bằng email và mật khẩu này.')
  } catch (err) {
