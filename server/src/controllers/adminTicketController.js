...
exports.getTickets = async (req, res) => {
  try {
    ...
    statusCounts.forEach((row) => { counts[row.status] = row._count._all })
...
exports.updateTicketStatus = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: 'Đã cập nhật trạng thái', data: ticket })
  } catch (err) {
    if (err.code === 'P2025') {
      return res.status(404).json({ success: false, message: 'Không tìm thấy yêu cầu liên hệ' })
    }
  ...
  try {
    ...
    if (assignedToId) {
      ...
      if (!assignee || !['STAFF', 'ADMIN'].includes(assignee.role)) {
        return res.status(400).json({ success: false, message: 'Người được phân công không hợp lệ' })
      }
    }
    ...
    if (assignedToId && ticket.status === 'NEW') {
      ...
      return res.json({ success: true, message: 'Đã phân công', data: updated })
    }
