...
const getAgeRangeLabel = (book) => {
  if (book.ageMin != null && book.ageMax != null)
    return `${book.ageMin}–${book.ageMax} tuổi`;
...
const buildWhere = async (query, exclude = {}) => {
  ...
  const featureList =
    !exclude.features && features
      ? String(features).split(",").filter(Boolean)
      : [];
  ...
  if (minAge) {
    andConditions.push({
      OR: [{ ageMax: null }, { ageMax: { gte: parseInt(minAge) } }],
    });
  }
  if (maxAge) {
    andConditions.push({
      OR: [{ ageMin: null }, { ageMin: { lte: parseInt(maxAge) } }],
    });
  }
...
const getFilterCounts = async (req, res) => {
  try {
    const [categories, whereNoCategory, whereNoFeatures, whereNoRating] =
      await Promise.all([
...
const getBook = async (req, res) => {
  try {
    ...
    ratingGroups.forEach((g) => {
      ratingBreakdown[g.rating] = g._count._all;
    });
    ...
    for (const g of voteGroups) {
      voteCountMap[g.reviewId] = voteCountMap[g.reviewId] || {
        helpfulCount: 0,
        unhelpfulCount: 0,
      };
  ...
  try {
    ...
    if (!purchasedFormats.length) {
      return formatResponse(
        res,
        403,
        "Bạn cần mua sách này (đơn hàng ở trạng thái thành công) mới có thể đánh giá",
      );
    }
    ...
    if (existing)
      return formatResponse(res, 400, "Bạn đã đánh giá sách này rồi");
...
const voteReview = async (req, res) => {
  try {
    ...
    const review = await prisma.review.findFirst({
      where: { id: reviewId, bookId: book.id, isVisible: true },
    });
    ...
    if (review.userId === req.user.id) {
      return formatResponse(
        res,
        400,
        "Không thể tự vote đánh giá của chính mình",
      );
    }
    ...
    if (!existing) {
      await prisma.reviewVote.create({
        data: { reviewId, userId: req.user.id, isHelpful },
      });
