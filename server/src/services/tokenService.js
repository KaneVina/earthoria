...
const REMEMBER_MS = parseDurationMs(
  process.env.JWT_REFRESH_REMEMBER_EXPIRES_IN || "30d",
);
const DEFAULT_MS = parseDurationMs(
  process.env.JWT_REFRESH_DEFAULT_EXPIRES_IN || "1d",
);
...
// Cho phép nhiều tab/request dùng chung 1 token
...
async function verifyAndRotate(rawToken, meta = {}) {
  ...
  // Ngoài grace window, hoặc không tìm được bản ghi sống hợp lệ nào trong chuỗi
